...
def PD_Vog_JC(Filename,dimension=1):
#    from ripser import Rips
#    
#    rips = Rips()
...
#    rips.plot(diagrams)
    ...
    dgms = ripser(D,distance_matrix=True, maxdim=dimension)['dgms']
    
    
    ######### MY CHANGES ###########
    # Modify H0 features: swap birth and death coordinates
    h0_transformed = np.array([[death, 0] for birth, death in dgms[0] if death < np.inf])

    # Combine with H1 (and possibly H2 if dimension > 1)
    transformed_dgms = [h0_transformed, dgms[1]]
    
    
    
    
    plot_diagrams(transformed_dgms, show=True)
#    a=ripser(D,distance_matrix=True)
#    plot_diagrams(a, show=True)


def VariancePersist_JC(Filename, pixelx=100, pixely=100, myspread=2, myspecs={"maxBD": 2, "minBD":0}, showplot=True):
    #Generate distance matrix and elementlist
    D,elements=Makexyzdistance(Filename)
    ...
    #Modifications to the original PI code

    # Iterate over columns until a column of zeros is found
    # imgs.shape[0] = rows, imgs.shape[1] = columns
    for col in range(imgs.shape[1]):
        if np.any(imgs[:, col] > 0):  # If the column has non-zero entries
            #Corr row = total number of rows - column index - 1
            corresponding_row = imgs.shape[0] - col - 1
            # Swap the column with the corresponding row
            imgs[:, col], imgs[corresponding_row, :] = imgs[corresponding_row, :].copy(), imgs[:, col].copy()
        else:
            break  # Stop when a zero column is encountered

    #Same plot code as the originall PI
    if showplot == True:
        fig, ax = plt.subplots()
        #cmap = get_cmap('twilight')
        img_plot = ax.imshow(imgs)#, cmap=cmap)
        plt.colorbar(img_plot, ax=ax, label='Intensity')
        ax.set_title('Transposed Persistence Image')
        ax.axis('off')
        
        #plt.legend(['H0', 'H1'], loc='upper right')
        plt.show()
        #pim.show(imgs)
        #plt.show()
    return np.array(imgs.flatten())
